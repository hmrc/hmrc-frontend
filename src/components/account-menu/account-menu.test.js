...
describe('/components/account-menu', () => {
  ...
  describe('When a page with an account-menu is loaded', () => {
    it('should display with a closed subnav on the page when loaded', async (done) => {
      ...
      /* ToDo: work out how to passs in the id as a variable
               subNavId = await page.evaluate(() => {
                 document.getElementById('account-menu__main-2').getAttribute('aria-owns'))
                } */

      const classList = await page.evaluate(() => {
        return document.getElementById('subnav-your-account').classList
      })
      const ariaHidden = await page.evaluate(() => {
        return document.getElementById('subnav-your-account').getAttribute('aria-hidden')
      })
      const ariaExpanded = await page.evaluate(() => {
        return document.getElementById('account-menu__main-2').getAttribute('aria-expanded')
      })
    ...
    it('Should have all mobile elements hidden', () => {
