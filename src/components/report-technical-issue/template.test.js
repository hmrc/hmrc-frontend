...
describe('Report Technical Issue', () => {
  ...
  it('should default to no Base URL', () => {
    ...
    expect($('.govuk-link').attr('href')).toEqual('http://localhost:9250/contact/problem_reports_nonjs?newTab=true&service=my-local-service');
  });

  it('should default to no Base URL', () => {
    const $ = render('report-technical-issue', examples['with-production-base-url']);

    expect($('.govuk-link').attr('href')).toEqual('https://www.tax.service.gov.uk/contact/problem_reports_nonjs?newTab=true&service=my-production-service');
  });
